    ...
    $(function () {
        ...
        function parseSarineInfos() {
            var attrNames = {
                ...
                sarineInfoDisplay: 'data-sarine-info-display'
            };

            $('[' + attrNames.sarineInfo + '], [' + attrNames.sarineInfoDisplay + ']').each(function () {
                var element = $(this),
                    field = element.attr(attrNames.sarineInfo) || element.attr(attrNames.sarineInfoDisplay),
                    ...
                    isShowDisplay = element.attr(attrNames.sarineInfoDisplay),
                    item = null;

                var filterDisplayName = function(prop){
                        return prop.name === value;
                    },
                    getDisplayName = function(item){
                        return item.hasOwnProperty('default-display') ? item['default-display'] : '';
                    };

                if(isShowDisplay){
                    if(gradeScales.hasOwnProperty(displayField)){
                        item = gradeScales[displayField].filter(filterDisplayName);
                        if(item.length > 0){
                            value = getDisplayName(item[0]);
                        }
                    }
                }
        ...
        function setProvidedLab() {
            ...
            if(flag && labName && labName.length > 0){
                $('[data-text="slide.summary.gradedBy"]').parent().css('display', 'block');
            }
        }
